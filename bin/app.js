...
var MainServer = function() {
	this.usernames = [];
	this.userNameOfSocket = new haxe_ds_ObjectMap();
...
MainServer.prototype = {
	...
	,sendClientJS: function(request,response) {
		response.sendfile("client.js");
	}
	,onConnection: function(socket) {
		var _g = this;
		socket.on("adduser",function(username) {
			_g.userNameOfSocket.set(socket,username);
			_g.usernames.push(username);
			socket.emit("updatechat","SERVER","you have connected");
			socket.emit("updatechat","SERVER",username + " has connected");
			_g.socketServer.sockets.emit("updateusers",_g.usernames);
		});
		socket.on("sendchat",function(data) {
			_g.socketServer.emit("updatechat",_g.userNameOfSocket.h[socket.__id__],data);
		});
...
js_npm_socketio_IAdapter.__name__ = true;
js_npm_socketio_IAdapter.prototype = {
	__class__: js_npm_socketio_IAdapter
};
